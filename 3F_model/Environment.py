...
class Environment():
    ...
    def make_csv(self, today=None):
        ...
        df1 = df1[['3F_temp','TA','CA','instant_headcount','cumulative_headcount','cnt']]
        if self.env_name == 'formula':
            df1['up'] = 0
            df1['down'] = 0
            for i in range(df1.shape[0]):
                if df1['3F_temp'].loc[i] >= df1['TA'].loc[i]:
                    df1['up'].loc[i] = df1['3F_temp'].loc[i] - df1['TA'].loc[i]
                else:
                    df1['down'].loc[i] = df1['TA'].loc[i] - df1['3F_temp'].loc[i]
            df1 = df1[['3F_temp','up','down','CA','instant_headcount','cumulative_headcount','cnt']].reset_index(drop=True)
        return df1
    ...
    def step(self,action):
        ...
        if next_ra[0,0] > 23:
            reward = 23 - next_ra[0,0]
        elif next_ra[0,0] < 22:
            reward = next_ra[0,0] - 22
        else:
            reward = 0
